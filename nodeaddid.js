...
function saveDeviceHash(deviceHash) {
    fs.appendFileSync(DEVICE_FILE, `${deviceHash}\n`, 'utf8');
}

async function switchState(state = 'ON') {
    try {
        const deviceHash = getRandomDeviceHash();
        saveDeviceHash(deviceHash);
